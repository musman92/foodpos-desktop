...
@section('title', 'Adjust Supplier Balance')

@section('content')
<div class="max-w-xl mx-auto">
    <div class="mb-6">
        <a href="{{ route('suppliers.show', $supplier) }}" class="text-indigo-600 hover:text-indigo-800 text-sm font-medium">
            <i class="fas fa-arrow-left mr-1"></i> Back to supplier
        </a>
    ...
    <div class="bg-white shadow rounded-lg p-6 space-y-6">
        <div class="rounded-lg bg-gray-50 border border-gray-200 p-4 text-sm">
            <p class="text-gray-600">Current balance: <span class="font-semibold text-gray-900">{{ format_currency($supplier->balance) }}</span></p>
            <p class="mt-1 text-gray-500">{{ \App\Support\PartyBalance::supplierStatusLabel((float) $supplier->balance) }}</p>
            <p class="mt-2 text-xs text-gray-500">{{ \App\Support\PartyBalance::supplierOpeningHint() }}</p>
        </div>

        <form action="{{ route('suppliers.balance-adjustment.store', $supplier) }}" method="POST" class="space-y-5">
            @csrf
            <div>
                ...
                <input type="number" name="new_balance" id="new_balance" step="0.01" required
                       value="{{ old('new_balance', $supplier->balance) }}"
            ...
            <div class="flex justify-end gap-3">
                <a href="{{ route('suppliers.show', $supplier) }}" class="px-4 py-2 h-12 border border-gray-300 rounded-lg text-sm font-medium text-gray-700 bg-white hover:bg-gray-50">Cancel</a>
