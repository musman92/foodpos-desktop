...
<div class="max-w-4xl mx-auto">
    <div class="bg-white shadow rounded-lg">
        ...
        <form action="{{ route('branches.update', $branch) }}" method="POST" class="p-6 space-y-6">
            @csrf
            @method('PUT')
            ...
            <div>
                ...
                <textarea name="address" 
                          ...
                          placeholder="Street address, City, State, ZIP">{{ old('address', $branch->address) }}</textarea>
                @error('address')
                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                @enderror
            </div>
                ...
                <div>
                    ...
                    <select name="timezone" 
                        ...
                        @foreach($timezones as $tz => $label)
                            <option value="{{ $tz }}" {{ old('timezone', $branch->timezone ?? $defaultTimezone ?? 'America/New_York') == $tz ? 'selected' : '' }}>
                                {{ $label }} ({{ $tz }})
                            </option>
                        @endforeach
                    </select>
