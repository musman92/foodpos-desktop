...
<div class="space-y-6">
    <div>
        ...
        <p class="mt-1 text-sm text-gray-500">Update role and permissions</p>
    </div>

    <form action="{{ route('roles.update', $role) }}" method="POST" class="bg-white shadow rounded-lg p-6 space-y-6">
        @csrf
        @method('PUT')
        <div>
            <label for="name" class="block text-sm font-medium text-gray-700 mb-2">Role name</label>
            <input type="text" name="name" id="name" value="{{ old('name', $role->name) }}" required
                   @if($isProtectedSystemRole) readonly @endif
                   class="block w-full max-w-md h-12 px-4 rounded-lg border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 @error('name') border-red-500 @enderror @if($isProtectedSystemRole) bg-gray-100 text-gray-600 cursor-not-allowed @endif">
            @if($isProtectedSystemRole)
                <p class="mt-1 text-xs text-gray-500">System roles cannot be renamed or deleted.</p>
            @endif
            @error('name')
        ...
        <div>
            ...
            @php $rolePermissionNames = $role->permissions->pluck('name')->toArray(); @endphp
