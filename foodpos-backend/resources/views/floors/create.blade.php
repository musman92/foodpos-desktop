...
<div class="max-w-2xl mx-auto space-y-6">
    <div>
        <h1 class="text-2xl font-bold text-gray-900">Create Floor</h1>
        <p class="mt-1 text-sm text-gray-500">Add a floor or level for dine-in layout</p>
    </div>

    <form action="{{ route('floors.store') }}" method="POST" class="bg-white shadow rounded-lg p-6 space-y-6">
        @csrf
        @if(show_branch_ui())
        <div>
        ...
        <div>
            ...
            <input type="text" name="name" id="name" value="{{ old('name') }}" required
                   ...
                   placeholder="e.g. Ground Floor, Patio, VIP Level">
            @error('name')
                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
            @enderror
        </div>
        ...
        <div class="flex items-center">
            ...
            <input type="checkbox" name="is_active" id="is_active" value="1" class="rounded border-gray-300 text-indigo-600 focus:ring-indigo-500" @checked(old('is_active', true))>
            <label for="is_active" class="ml-2 text-sm text-gray-700">Active</label>
        </div>
        <div class="flex items-center gap-3 pt-4 border-t border-gray-200">
            <a href="{{ route('floors.index', ['branch_id' => $branchId]) }}" class="px-4 py-2 border border-gray-300 rounded-lg text-sm font-medium text-gray-700 bg-white hover:bg-gray-50">Cancel</a>
            <button type="submit" class="px-4 py-2 bg-indigo-600 text-white rounded-lg text-sm font-medium hover:bg-indigo-700">Create Floor</button>
        </div>
    </form>
</div>
@endsection
